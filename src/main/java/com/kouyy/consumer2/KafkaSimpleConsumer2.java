...
import com.kouyy.consumer1.PartitionMsgTask;
 ...
 * 指定topic的消费者
 */
public class KafkaSimpleConsumer2 {
    private static final Logger logger = LogManager.getLogger(KafkaSimpleConsumer2.class);
    ...
    public KafkaSimpleConsumer2(KafkaConsumerConfig config, String topic) {
    ...
    public void run() {
        ...
        this.executor = Executors.newFixedThreadPool(this.partitionNum);
            for (int threadNum = 0; threadNum < this.partitionNum; threadNum++) {
                this.executor.execute(new JavaKafkaSimpleConsumerAPI(this.kafkaConfig, threadNum));
            }
    }
