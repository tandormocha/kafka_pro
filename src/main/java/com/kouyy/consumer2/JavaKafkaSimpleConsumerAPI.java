...
/**
 * 利用fetch方法发送请求broker获取元数据
 */
public class JavaKafkaSimpleConsumerAPI implements Runnable {
    // 最大重试次数
    private int maxRetryTimes = 5;
    // 重试间隔时间
    private long retryIntervalMillis = 1000;
    // 缓存Topic/Partition对应的Broker连接信息
    private Map<KafkaTopicPartitionInfo, List<KafkaBrokerInfo>> replicaBrokers = new ConcurrentHashMap<KafkaTopicPartitionInfo, List<KafkaBrokerInfo>>();
...
//    private KafkaConsumerWork consumerWork;
    private KafkaTopicPartitionInfo topicPartitionInfo;
    private List<KafkaBrokerInfo> seedBrokers;
    private static Charset charset = Charset.forName("UTF-8");
    private static CharsetDecoder decoder = charset.newDecoder();
    private static String charsetName = "UTF-8";

    public JavaKafkaSimpleConsumerAPI(KafkaConfig config, int partitionID) {
...
//        this.consumerWork = ((KafkaConsumerWork) SpringContextUtil.getBean("kafkaConsumerWork"));
        this.seedBrokers = new ArrayList<KafkaBrokerInfo>();
        for (int i = 0; i < kafkaConfig.replicaBrokers.size(); i++) {
            KafkaBrokerInfo KafkaBrokerInfo = new KafkaBrokerInfo(kafkaConfig.replicaBrokers.get(i), kafkaConfig.port);
            this.seedBrokers.add(KafkaBrokerInfo);
        }
        topicPartitionInfo = new KafkaTopicPartitionInfo(topic, partitionID);
    }

    @Override
    public void run() {
        try {
            Thread.currentThread().setName("Thread-" + topicPartitionInfo.topic + "-partition" + topicPartitionInfo.partitionID);
            this.subsribe(-1, topicPartitionInfo, seedBrokers);
        } catch (Exception ex) {
            ex.printStackTrace();
            logger.error(ex);
        }
     ...
     * @throws Exception
     */
    public void subsribe(long maxReads, KafkaTopicPartitionInfo topicPartitionInfo, List<KafkaBrokerInfo> seedBrokers) throws Exception {
        ...
        try {
            ...
            long readOffSet = -1;
            while (true) {
                readOffSet = this.getLastOffSet(consumer, groupId, topic, partitionID, whichTime, clientName, false);
                if (readOffSet == -1) {
                    // 当返回为-1的时候，表示异常信息
            ...
            logger.info(topic + "partition" + partitionID + " first read offset is:" + readOffSet);
            int numErrors = 0;
            boolean ever = maxReads <= 0;
            long lastReadOffSet = readOffSet;
            ...
            while (ever || maxReads > 0) {
                ...
                request = new FetchRequestBuilder().clientId(clientName).addFetch(topic, partitionID, readOffSet, 1000000).build();
                // 发送请求到Kafka，并获得返回值
                response = consumer.fetch(request);
                // 如果返回对象表示存在异常，进行异常处理，并进行consumer重新连接的操作
                // 当异常连续出现次数超过5次的时候，程序休眠6秒钟
                if (response.hasError()) {
                    String leaderBrokerHost = consumer.host();
                    short code = response.errorCode(topic, partitionID);
                    numErrors++;
                    logger.warn(topic + "partition" + partitionID + " Error fetching data from the Broker:" + leaderBrokerHost + " Reason:" + code + " numErrors:" + numErrors);
                    if (code == ErrorMapping.OffsetOutOfRangeCode()) {
                        // 异常表示是offset异常，重新获取偏移量即可
                        Thread.sleep(this.retryIntervalMillis);
                        if (numErrors > 5) {
                            Thread.sleep(this.retryIntervalMillis * 6);
                            readOffSet = this.getLastOffSet(consumer, groupId, topic, partitionID, kafka.api.OffsetRequest.LatestTime(), clientName, true);
                        } else {
                            readOffSet = this.getLastOffSet(consumer, groupId, topic, partitionID, kafka.api.OffsetRequest.LatestTime(), clientName, false);
                        }
                        continue;
                    }
                    consumer.close();
                ...
                for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
                    ...
                    // 读取数据的value
                    payload = messageAndOffset.message().payload();
                    bytes = new byte[payload.limit()];
                    payload.get(bytes);
                    message = new String(bytes, charsetName);

                    //String message=  byteBufferToString(payload);
                    try {
//                        this.consumerWork.Work(message);
                ...
                if (numRead == 0) {
                    try {
                        Thread.sleep(this.retryIntervalMillis);
                        if (lastReadOffSet != readOffSet) {
                            logger.warn("Offset:" + topic + ":" + partitionID + ":" + readOffSet + " Reads:" + numRead + "条");
                        }
                    } catch (Exception e) {
                        // nothings
                    }
                    ...
                    sb.append(numRead);
                    sb.append("条");
                    //logger.info("Offset:" + topic + ":" + partitionID + ":" + readOffSet + " Reads:" + numRead + "条");
    ...
    public PartitionMetadata findLeader(List<KafkaBrokerInfo> brokers, String topic, int partitionID) {
        ...
        for (KafkaBrokerInfo broker : brokers) {
            ...
            try {
                ...
                logger.error("Error communicating with Broker [" + broker.brokerHost + "] to find Leader for [" + topic + ", " + partitionID + "] Reason:" + e);
            } finally {
                if (consumer != null) {
                    try {
                        consumer.close();
     ...
     */
    public long getLastOffSet(SimpleConsumer consumer, String groupId, String topic, int partitionID, long whichTime, String clientName, boolean isSkipZookeeper) {
    ...
    public long getOffsetOfTopicAndPartition(SimpleConsumer consumer, String groupId, String clientName, String topic, int partitionID) {
        ...
        if (returnOffsetMetadata != null && !returnOffsetMetadata.isEmpty()) {
            ...
            if (offset.error() == ErrorMapping.NoError()) {
                ...
                logger.error("Error fetching data Offset Data the Topic and Partition. Reason: " + offset.error());
            }
        }
